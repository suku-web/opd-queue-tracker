...
router.put('/checkin/:id/served', auth, async (req, res) => {
  try {
    ...
    // Count remaining waiting patients
    const remaining = await CheckIn.countDocuments({
    ...
    // Socket.io room
    const roomId =
      checkIn.hospitalId + '_' + checkIn.department

    // Send real-time update
    io.to(roomId).emit('queue_updated', {
      queueCount: remaining,
      estimatedWait: remaining * avgTime
    })
